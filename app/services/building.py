...
class BuildingService:
    ...
    async def create(self, address: str, latitude: float, longitude: float) -> Building:
        return await self.building_repo.create(address=address, latitude=latitude, longitude=longitude)

    async def update(self, building: Building, address: str = None, latitude: float = None, longitude: float = None) -> Building:
        return await self.building_repo.update(building, address=address, latitude=latitude, longitude=longitude)
