...
class ActivityService:
    ...
    async def update(self, activity: Activity, name: str = None, parent_id: int = None) -> Activity:
        if parent_id is not None:
            parent = await self.activity_repo.get_by_id(parent_id)
            if activity.id == parent_id:
                raise HTTPException(status_code=400, detail="Запись не может зависеть от себя")
            if not parent:
                raise HTTPException(status_code=400, detail="Указанный родительский элемент не существует")
            if await self.activity_repo.get_depth(parent.id) >= 2:
                raise HTTPException(status_code=400, detail="Превышен лимит вложенности. Максимальный уровень - 3")
